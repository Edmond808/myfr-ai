...
export const CATEGORY_EXAMPLES: Record<Category, string> = {
  groceries: EXAMPLES[0],
  housekeeping: EXAMPLES[1],
  boats: EXAMPLES[2],
  transport: EXAMPLES[3],
  rentals: EXAMPLES[4],
  events: EXAMPLES[5],
  handyman: "Need a handyman to fix a leaking tap in my villa in Antibes…",
  concierge: "Need help arranging restaurant reservations and spa bookings in Monaco…",
};
...
> = {
  groceries: { label: "Groceries & errands", icon: ShoppingBasket },
  housekeeping: { label: "Housekeeping & cleaning", icon: Sparkles },
  rentals: { label: "Rentals & accommodation", icon: Home },
  transport: { label: "Transport & transfers", icon: Car },
  boats: { label: "Boats & charters", icon: Anchor },
  handyman: { label: "Handyman & works", icon: Hammer },
  events: { label: "Events & private chefs", icon: CalendarHeart },
  concierge: { label: "Concierge & other", icon: ConciergeBell },
};

export const MERCHANTS: Record<Category, Merchant[]> = {
  groceries: [
