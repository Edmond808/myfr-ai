import type { Category, JobClassification, Urgency } from "../src/lib/types";
...
const KEYWORDS: Record<Category, string[]> = {
  groceries: ["grocery", "groceries", "shopping", "errand", "market", "courses"],
  housekeeping: ["housekeep", "clean", "cleaning", "maid", "villa care"],
  rentals: ["rent", "rental", "apartment", "stay", "accommodation", "month"],
  transport: ["transfer", "airport", "driver", "vtc", "taxi", "car"],
  boats: ["boat", "yacht", "charter", "skipper", "marine"],
  handyman: ["handyman", "fix", "repair", "works", "plumb", "electric"],
  events: ["chef", "dinner", "catering", "event", "party", "soirée"],
  concierge: ["concierge", "assist", "help", "other"],
};
...
function detectUrgency(text: string): Urgency {
  const lower = text.toLowerCase();
  if (
    /today|tonight|asap|urgent|this morning|this afternoon/.test(lower)
  ) {
    return "today";
  }
  if (/this week|tomorrow|weekend|friday|saturday|sunday/.test(lower)) {
    return "this_week";
  }
  return "flexible";
}
